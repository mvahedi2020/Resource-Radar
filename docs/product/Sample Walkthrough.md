...
If the saved payload is malformed, Apply remains blocked until the user explicitly resets the sample, preserving the recovery boundary. A browser storage access failure is handled separately: the current tab remains available for inspection, but later persistence may be unavailable.
