...
| Scenarios | Reset sample | Asks for confirmation, then restores original fictional fixtures and removes the saved local plan | A visible one-step recovery restores both the prior applied baseline and working draft; no shared data is changed |
